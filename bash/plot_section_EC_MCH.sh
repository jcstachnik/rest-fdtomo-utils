...
sgrd=/data/TOPO/Slab2.0/Slab2_South_America_Region/sam_slab2_dep_02.23.18.grd #slab2.0 grd
...
c1=`echo $infil | awk -F. '{printf "%s.%03d", $2,$3}'`
...
if [ "$plotcont" -eq 1 ]; then
    pscontour kmyv.txt -W0.4p $J $R -C${cont1} -O -K $parms >> $outps
    pscontour kmyv.txt -W0.4p $J $R -C${cont2} -Wc0.8p -O -K $parms >> $outps
else
...
##psxy command needs the $params variable to plot correctly
     psxy slab.xy $J $R -Wthin,black,---- -O -K $parms >> $outps
fi
if [ -f $evtfile ]; then
    awk '$1==2012 || $1==2013 || $1==2014 || $1==2015 || $1==2016 {print $7,$6,$8}' $evtfile |\
     project -C$lo0/$la0 -E$lo1/$la1 -Fpz -Q -W-${wid}/${wid} |\
     psxy $J $R -Sc0.01i -Gwhite -Wblack -O -K $parms >> $outps
...
if [ -f $hitfile ]; then
    ...
    project -C$lo0/$la0 -E$lo1/$la1 -Fxyzp -Q |\
    awk '{print $5,$3,$4}' |\
     pscontour $J $R -C+10 -W0.4p,white -O -K $parms >> $outps
...
psbasemap -JX${xinch}i/0.3i -R0/${xmx2}/$tymn/4 \
 -Bpyf0.5a2 -BW \
 -O -K -Y${yo}i \
 --FONT_ANNOT_PRIMARY=6p \
 --PROJ_LENGTH_UNIT=in >> $outps
psxy -JX${xinch}i/0.3i -R0/${xmx2}/$tymn/4 -G135/206/250 -N -O -K <<EOF >> $outps
0 0
$xmx2 0
$xmx2 -6
0 -6
EOF
psxy topo1.xy -JX${xinch}i/0.3i -R0/${xmx2}/$tymn/4 -G210 -N -O -K >> $outps
psxy topo1.xy -JX${xinch}i/0.3i -R0/${xmx2}/$tymn/4 -Wthin,black -N -O -K >> $outps
...
if [ -f $stafile ]; then
    ...
    awk '{print $1,$2/1000}' |\
    psxy -JX${xinch}i/0.3i -R0/${xmx2}/$tymn/4 -St0.1i \
